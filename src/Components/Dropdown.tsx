...
// Define the props that the Dropdown component will receive
interface Dropdown {
  ...
  return (
    <div className="relative inline-block text-left">
      ...
      <Menu as="div" className="relative">
        <div>
                    {/* Button that displays the selected product */}
          <Menu.Button className="inline-flex w-full justify-between px-4 py-2 text-sm font-medium text-white bg-gray-800 rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            {selectedProduct || 'Select a product'} {/* Display selected product */}
          ...
          <div className="p-1">
            {productOptions.map(option => (
              <Menu.Item key={option.name}>
                {({ active }) => (
                  <button
                    onClick={() => setSelectedProduct(option.name)}  //Update selected product when clicked
